...
ren <- import(snakemake@input[["ren"]])
wellcome <- import(snakemake@input[["wellcome"]])
combat <- import(snakemake@input[["combat"]])
ch <- import(snakemake@input[["ch"]])
glaucoma <- import(snakemake@input[["glaucoma"]])
ra <- import(snakemake@input[["ra"]])

# Example combining multiple datasets
summary_all <- rbind(
  make_summary_row(ren, "Ren et al."),
  make_summary_row(wellcome, "Wellcome"),
  make_summary_row(combat, "COMBAT"),
  make_summary_row(ch, "CH"),
  make_summary_row(glaucoma, "Glaucoma"),
  make_summary_row(ra, "RA")
)
