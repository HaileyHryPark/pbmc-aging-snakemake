...
mat <- AverageExpression(seu, assays = "RNA", group.by = c("donor_id","predicted.celltype.l1"), features = rownames(seu), return.seurat = FALSE)$RNA
...
final_mat$age <- as.integer(as.character(meta[rownames(final_mat), "age"]))
...
final_mat <- final_mat %>% rownames_to_column("donor_id")
final_mat$sample_id <- final_mat$donor_id
print(final_mat[,c("sample_id", "donor_id","age","sex","dataset","ethnicity")])
