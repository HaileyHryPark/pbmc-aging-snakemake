...
#export(center %>% as.data.frame() %>% rownames_to_column(), snakemake@output[["center1"]])
#export(clust$membership %>% as.data.frame() %>% rownames_to_column(), snakemake@output[["mem1"]])
...
#mfuzz.plot(eset = eset, min.mem = 0.5, cl = clust, mfrow = c(1,5), time.labels = eset@phenoData@data$time, new.window = F)
orig_clusters <- lapply(seq_len(cnum), function(k) {
  names(which(clust$cluster == k))
})
...
high_cor_pairs <- which(cor_mat > 0.8 & upper.tri(cor_mat), arr.ind = TRUE)

# Merge into groups based on correlation
merge_groups <- list()
...
# For each variable, find all merged clusters corresponding to those original clusters
var_merged_list <- mapply(function(orig_str) {
  if (is.na(orig_str)) return(NA)
  orig_clusters <- unlist(strsplit(orig_str, ";"))
  merged <- unique(cluster_map[orig_clusters])
...
# ----- 3. Plot merged clusters with consistent y-axis -----
...
plot_clusters(eset, merged_clusters, main_prefix = "Merged Cluster")

dev.off()

# ----- 4. Correlation plot for merged clusters -----

# Compute mean profile for each merged cluster
merged_centers <- sapply(unique(cluster_map), function(merged_label) {
  vars_in_cluster <- var_cluster_df$variable[
    grepl(merged_label, var_cluster_df$merged_clusters)
  ]
  if (length(vars_in_cluster) == 0) {
    return(rep(NA, ncol(eset)))
  }
  colMeans(eset@assayData$exprs[vars_in_cluster, , drop = FALSE])
})

# Make sure it's a matrix with proper row/col names
merged_centers <- t(merged_centers)
rownames(merged_centers) <- unique(cluster_map)

# Correlation matrix
merged_cor <- cor(t(merged_centers))
