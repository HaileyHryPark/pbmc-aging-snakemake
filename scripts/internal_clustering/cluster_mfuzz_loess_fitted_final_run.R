...
feature_initial_df <- data.frame(
  feature = names(cl$cluster),
  membership = max_membership[names(cl$cluster)],
  initial_cluster = paste0("Cluster_", as.character(cl$cluster)),
...
feature_initial_df <- feature_initial_df[
  order(feature_initial_df$merged_cluster,
        feature_initial_df$initial_cluster,
        feature_initial_df$feature),
]

export(
  feature_initial_df,
  snakemake@output[["final_cluster_df"]]
)
