...
deg_plot_data <- bind_rows(deg_summary, deg_total) %>% mutate(celltype = factor(celltype, levels = c("All","CD4 Naive","CD4 TCM","CD4 TEM","CD8 Naive","CD8 TCM","CD8 TEM")))
...
pdf(snakemake@output[["plots2"]], width = 13, height = 8.6)
	...
	p2 <- deg_plot_data %>% filter(celltype != "All") %>% ggplot(aes(x = age_threshold, y = n_DEGs, color = gender)) +
		...
		geom_point(size = 1.5) +
  		scale_color_manual(values = c("Both" = "grey", "Female" = "#E15566", "Male" = "#4981BF")) +
  		facet_wrap(~ celltype, nrow = 2, ncol = 3, scales = "free")+
  		labs(
    			x = "Age (years)",
    			y = "Number of DEFs",
    			color = "Sex"
